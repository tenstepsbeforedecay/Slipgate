...
const App = () => {
    ...
    useEffect(() => {
        const handleShowError = (_e, title, message) => {
    ...
    useEffect(() => {
        ...
        const handleVisibilityIpc = () => clearStuckState();
    ...
    _jsxs(SidebarProvider, { defaultOpen: false, className: "relative w-full h-screen overflow-hidden", style: { backgroundColor: resolvedTheme === 'dark' ? '#000000' : '#E5E5E5' }, children: [_jsx("img", { src: mapBg, alt: "", className: `pointer-events-none absolute inset-0 opacity-65 w-full h-full object-cover z-0 transition-[filter] duration-500 ${isHome ? '' : 'blur-3xl'}` }), platform === 'darwin' && (_jsx("div", { className: "fixed top-0.5 -left-1 h-14.25 flex items-center pl-3 z-100 app-drag", children: _jsx(WindowControls, {}) })), _jsx(AppSidebar, {}), _jsx("div", { className: "relative z-10 main grow h-full overflow-y-auto", children: page }), _jsx(AppUpdateOverlay, {})] }));
};
export default App;
