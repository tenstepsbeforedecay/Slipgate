export default {
    ...
    guide: {
        ...
        insertLinkDesc: '手动导入：点击右侧粘贴按钮并填入订阅链接。链接有效后会自动进入下一步。深链导入：若已出现确认弹窗，该步骤也会自动继续。',
        validLinkRequired: '请先输入有效的 http/https 链接后再继续。',
        importProfileTitle: '导入订阅',
        importProfileDesc: '手动导入请点击“导入”；深链导入请先在弹窗中确认导入。导入成功后会自动进入下一步。',
        ...
        welcomeDesc: '这是一份交互式使用教程，如果您已经完全熟悉本软件的操作，可以直接点击右上角关闭按钮，后续您可以随时从设置中打开本教程',
        navbar: '导航栏',
        navbarDesc: '左侧是应用的导航栏，兼顾仪表盘功能，在这里可以切换不同页面，也可以概览常用的状态信息',
        card: '卡片',
        cardDesc: '点击导航栏卡片可以跳转到对应页面，拖动导航栏卡片可以自由排列卡片顺序',
        mainArea: '主要区域',
        mainAreaDesc: '右侧是应用的主要区域，展示了导航栏所选页面的内容',
        profileManagement: '订阅管理',
        profileManagementDesc: '订阅管理卡片展示当前运行的订阅配置信息，点击进入订阅管理页面可以在这里管理订阅配置',
        profileImport: '订阅导入',
        profileImportDesc: 'Slipgate 支持多种订阅导入方式，在此输入订阅链接，点击导入即可导入您的订阅配置，如果您的订阅需要代理才能更新，请勾选"代理"再点击导入，当然这需要已经有一个可以正常使用的订阅才可以',
        localProfile: '本地订阅',
        localProfileDesc: '点击"+"可以选择本地文件进行导入或者直接新建空白配置进行编辑',
        sysProxy: '系统代理',
        sysProxyDesc: '导入订阅之后，内核已经开始运行并监听指定端口，此时您已经可以通过指定代理端口来使用代理了，如果您要使大部分应用自动使用该端口的代理，您还需要打开系统代理开关',
        sysProxySettings: '系统代理设置',
        sysProxySettingsDesc: '在此您可以进行系统代理相关设置，选择代理模式，如果某些 Windows 应用不遵循系统代理，还可以使用"UWP 工具"解除本地回环限制，对于"手动代理模式"和"PAC 代理模式"的区别，请自行百度',
        tun: '虚拟网卡',
        tunDesc: '虚拟网卡，即同类软件中常见的"Tun 模式"，对于某些不遵循系统代理的应用，您可以打开虚拟网卡以让内核接管所有流量',
        tunSettings: '虚拟网卡设置',
        tunSettingsDesc: '这里可以更改虚拟网卡相关设置，Slipgate 理论上已经完全解决权限问题，如果您的虚拟网卡仍然不可用，可以尝试重设防火墙（Windows）或手动授权内核（MacOS/Linux）后重启内核',
        dnsDesc: '软件默认接管了内核的 DNS 设置，如果您需要使用订阅配置中的 DNS 设置，可以到应用设置中关闭"接管 DNS 设置"，域名嗅探同理',
        tutorialEnd: '教程结束',
    ...
    modal: {
        ...
        adminRequiredDesc: '应用需要以管理员身份运行一次以设置任务计划。设置完成后，应用将自动以提升的权限启动。',
    ...
    settings: {
        ...
        advanced: {
            ...
            useHotReloadProfileHelp: '启用后切换或保存订阅时通过 API 热重载配置而无需重启内核，禁用时仍使用重启方式切换',
