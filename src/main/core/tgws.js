...
// ---- defaults --------------------------------------------------------------
const DEFAULT_DC_IPS = ['2:149.154.167.220', '4:149.154.167.220'];
...
async function ensureSecret(current) {
...
function dcIpsToTargets(dcList) {
    const ips = [];
...
async function waitForNetwork(targets) {
...
export function startTgws() {
    return withLock(() => startTgwsImpl());
}
export function stopTgws() {
    return withLock(() => stopTgwsImpl());
}
async function startTgwsImpl() {
    if (child) {
    ...
    if (!cfg0.tgws)
        throw new Error('tgws config missing');
    ...
    try {
        ...
        for (const d of dcIp)
            args.push('--dc-ip', d);
        if (t.bufKb)
            args.push('--buf-kb', String(t.bufKb));
        if (t.poolSize)
            args.push('--pool-size', String(t.poolSize));
        if (t.verbose)
            args.push('-v');
        if (t.cfproxy === false)
            args.push('--no-cfproxy');
        if (t.cfproxyUserDomain)
            args.push('--cfproxy-domain', t.cfproxyUserDomain);
        if (t.fakeTlsDomain)
            args.push('--fake-tls-domain', t.fakeTlsDomain);
...
export async function restartTgws() {
...
export async function getTgwsLink() {
    const cfg = await getAppConfig();
    const t = cfg.tgws;
    if (!t)
        return '';
