...
export async function checkAppUpdate(force = false) {
    ...
    if (!force && cache && Date.now() - cache.at < CACHE_TTL_MS && cache.data.tag) {
        if (Date.now() - cache.at > 30 * 60 * 1000)
            backgroundRefresh();
    ...
    try {
        ...
        throw new Error(`Не удалось проверить обновления Slipgate: ${e instanceof Error ? e.message : String(e)}`);
    }
    if (release.draft || release.prerelease) {
        const info = { installed, hasUpdate: false };
    ...
    const installerAsset = assets.find((a) => /^Slipgate_x64\.exe$/i.test(a.name)) ??
...
export async function dismissAppUpdate(tag) {
    if (!tag)
        return;
 ...
 */
export async function installAppUpdate(assetUrl, expectedVersion) {
    if (!assetUrl)
        throw new Error('Пустая ссылка на установщик');
...
export function silentBackgroundCheck() {
...
export function invalidateAppUpdateCache() {
    cache = null;
}
