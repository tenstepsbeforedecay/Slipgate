...
export function getZapretStatus() {
    return status;
}
/**
...
export async function installZapretBundle(zipBytes) {
    ...
    if (process.platform === 'win32') {
        for (const name of WINDIVERT_SERVICE_NAMES) {
            try {
                spawnSync('sc.exe', ['stop', name], { windowsHide: true, timeout: 3000 });
            }
            catch { /* noop */ }
            try {
                spawnSync('sc.exe', ['delete', name], { windowsHide: true, timeout: 3000 });
            }
            catch { /* noop */ }
        }
    ...
    const normalize = (n) => n.replace(/\\/g, '/');
    const generalEntry = entries.find((e) => !e.isDirectory && /(^|\/)general\.bat$/i.test(normalize(e.entryName)));
    if (!generalEntry) {
    ...
    const hasBin = entries.some((e) => !e.isDirectory && normalize(e.entryName).toLowerCase() === expectedBin.toLowerCase());
    if (!hasBin) {
    ...
    for (const e of entries) {
        ...
        if (rootPrefix && !name.startsWith(rootPrefix))
            continue;
    ...
    if (preservedListGeneral) {
        ...
        const splitLines = (buf) => buf.toString('utf-8').split(/\r?\n/).map((l) => l.trim());
    ...
    log('info', `Zapret bundle installed: ${written} files, ${list.length} strategies` +
...
export function withZapretLock(fn) {
    return withLock(fn);
}
 ...
 */
export function ensureWinDivertReady() {
 ...
 */
let winwsHealth = 'unknown';
let winwsLastError = null;
function resetWinwsHealth() {
    winwsHealth = 'unknown';
    winwsLastError = null;
}
function ingestWinwsLine(raw) {
    ...
    if (/windivert.*failed|windivertopen.*failed|driver.*not.*loaded|access\s+is\s+denied|requires.*admin/i.test(raw)) {
        winwsHealth = 'failed';
        winwsLastError = raw.trim();
    }
}
async function startZapretImpl() {
    if (child)
        return;
    ...
    while (Date.now() < deadline) {
        if (!child)
            return; // cmd already died — exit handler will set error
...
export async function restartZapret() {
    await stopZapret();
