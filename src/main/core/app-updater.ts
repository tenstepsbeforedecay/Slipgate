...
export async function installAppUpdate(
  ...
  // Re-launch watcher. With `oneClick: false` (assisted installer)
  if (installerPid) {
    try {
      ...
      const watcherScript = [
        ...
        // Poll for up to 5 minutes — silent NSIS upgrades typically take
        // 5–15s but we leave a generous ceiling for slow disks / AV scans.
        ...
        '}',
        // Brief grace period so antivirus / Windows Defender finishes
        // its post-write scan and releases the new Slipgate.exe handle.
        'Start-Sleep -Seconds 1',
        'if (Test-Path -LiteralPath $exe) {',
        '  Start-Process -FilePath $exe',
        '}'
      ].join('\n')
      const watcherPath = path.join(dir, `Slipgate-relaunch-${Date.now()}.ps1`)
