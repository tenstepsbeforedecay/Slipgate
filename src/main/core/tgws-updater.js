...
export async function checkTgwsUpdate(force = false) {
    ...
    const winAsset = assets.find((a) => /^TgWsProxy_windows\.exe$/i.test(a.name)) ??
...
async function killStaleTgwsBinary() {
...
export async function installTgwsUpdate(assetUrl, expectedVersion) {
    if (!assetUrl)
        throw new Error('Пустая ссылка на бинарник');
...
export async function dismissTgwsUpdate(tag) {
    if (!tag)
        return;
    const cfg = await getAppConfig();
    const next = {
