...
app.whenReady().then(async () => {
    ...
    if (process.platform === 'win32' && !is.dev && !(await isRunningAsAdmin())) {
        dialog.showErrorBox('Slipgate — нужны права администратора', 'Slipgate должен быть запущен с правами администратора, иначе Zapret\n' +
    ...
    const uiTasks = [initShortcut()];
    if (!appConfig.disableTray)
 ...
 */
function syncKillChildren() {
    if (process.platform !== 'win32')
        return;
    try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { spawnSync } = require('child_process');
        const opts = { windowsHide: true, timeout: 2000 };
...
export async function createWindow(appConfig) {
    ...
    mainWindow.on('ready-to-show', () => {
        ...
        if (silentStart && !config.disableTray)
            return;
        mainWindow?.show();
        mainWindow?.focus();
    });
    ...
    const onWindowVisibilityChange = (kind) => {
...
export async function triggerMainWindow() {
