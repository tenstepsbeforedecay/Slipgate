...
function isValidConfig(c) {
    return !!c && typeof c === 'object' && 'appTheme' in c;
}
async function safeWriteConfig(content) {
...
export async function getAppConfig(force = false) {
    ...
    if (!appConfig || typeof appConfig !== 'object')
        appConfig = defaultConfig;
    const persistedVersion = typeof appConfig.configVersion === 'number'
    ...
    appConfig = deepMerge(defaultConfig, appConfig);
    if (appConfig.appTheme !== 'light' && appConfig.appTheme !== 'dark') {
...
export async function patchAppConfig(patch) {
    ...
    const disableTrayChanged = patch.disableTray !== undefined && patch.disableTray !== prevDisableTray;
    const hideChanged = patch.hideTaskbarIcon !== undefined && patch.hideTaskbarIcon !== prevHideTaskbarIcon;
