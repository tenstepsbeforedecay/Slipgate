...
import { getZapretStatus, startZapret, stopZapret, restartZapret, listStrategies, installZapretBundle } from '../core/zapret';
import { checkZapretUpdate, installZapretUpdate, dismissZapretUpdate } from '../core/zapret-updater';
import { runStrategyTests, getStrategyTestResults, isStrategyTestRunning } from '../core/zapret-tester';
import { getCuratedIpSets, getIpListSnapshot, applyIpListPatch, clearIpList, restoreIpListBackup } from '../core/zapret-iplist';
import { getGameFilterMode, setGameFilterMode, getIpsetFilterSnapshot, setIpsetFilterMode, updateIpsetList } from '../core/zapret-service-settings';
import { checkTgwsUpdate, installTgwsUpdate, dismissTgwsUpdate } from '../core/tgws-updater';
import { checkAppUpdate, installAppUpdate, dismissAppUpdate } from '../core/app-updater';
import { execFile } from 'node:child_process';
import { writeFileSync, unlinkSync } from 'node:fs';
import path from 'node:path';
//
function logTgError(stage, err, stderr) {
    if (!err && !stderr)
        return;
    const msg = err?.message ?? stderr ?? 'unknown';
    console.error(`[openTelegramLink] ${stage}: ${msg.trim()}`);
}
function tryFallbackOpen(url) {
    shell.openExternal(url).catch((e) => logTgError('shell.openExternal', e));
}
function openTgLinkViaScheduler(url, fellBack) {
    try {
        ...
        execFile('schtasks.exe', [
            ...
            '/IT',
            '/RL', 'LIMITED'
        ], { windowsHide: true }, (createErr, _stdout, createStderr) => {
            if (createErr) {
            ...
            execFile('schtasks.exe', ['/Run', '/TN', taskName], { windowsHide: true }, (runErr, _so, runStderr) => {
                if (runErr) {
        ...
        logTgError('openTgLinkViaScheduler sync', e);
        return false;
    }
}
...
export function registerIpcMainHandlers() {
    ...
    ipcMain.handle('app:patchConfig', h((patch) => patchAppConfig(patch)));
    ipcMain.handle('app:version', h(() => app.getVersion()));
    // ---- Theme --------------------------------------------------------------
    ipcMain.handle('theme:setNative', h((theme) => setNativeTheme(theme)));
    ipcMain.handle('theme:apply', h((file) => applyTheme(file)));
    // ---- Utility ------------------------------------------------------------
    ipcMain.handle('shell:openTelegramLink', h((url) => openTelegramLink(url)));
    ipcMain.handle('clipboard:writeText', h((text) => { clipboard.writeText(text); }));
    // ---- TG WS Proxy --------------------------------------------------------
    ...
    ipcMain.handle('tgws:installUpdate', h((url, expectedVersion) => installTgwsUpdate(url, expectedVersion)));
    ipcMain.handle('tgws:dismissUpdate', h((tag) => dismissTgwsUpdate(tag)));
    // ---- Zapret -------------------------------------------------------------
    ...
    ipcMain.handle('zapret:installBundle', h((bytes) => installZapretBundle(bytes)));
    ipcMain.handle('zapret:checkUpdate', h((force) => checkZapretUpdate(Boolean(force))));
    ipcMain.handle('zapret:installUpdate', h((url, expectedVersion) => installZapretUpdate(url, expectedVersion)));
    ipcMain.handle('zapret:dismissUpdate', h((tag) => dismissZapretUpdate(tag)));
    ...
    ipcMain.handle('zapret:applyIpListPatch', h((patch) => applyIpListPatch((patch ?? {}))));
    ...
    ipcMain.handle('zapret:setGameFilter', h((mode) => setGameFilterMode(mode)));
    ipcMain.handle('zapret:getIpsetFilter', h(() => getIpsetFilterSnapshot()));
    ipcMain.handle('zapret:setIpsetFilter', h((mode) => setIpsetFilterMode(mode)));
    ...
    ipcMain.handle('app:installUpdate', h((url, expectedVersion) => installAppUpdate(url, expectedVersion)));
    ipcMain.handle('app:dismissUpdate', h((tag) => dismissAppUpdate(tag)));
    // ---- Quit / restart -----------------------------------------------------
    ...
    ipcMain.handle('setLanguage', async (_e, _lang) => undefined);
