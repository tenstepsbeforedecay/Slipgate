...
export async function initShortcut() {
    ...
    if (cfg.showWindowShortcut) {
        try {
            globalShortcut.register(cfg.showWindowShortcut, () => triggerMainWindow());
        }
        catch { /* noop */ }
    }
    if (cfg.tgwsToggleShortcut) {
        try {
            globalShortcut.register(cfg.tgwsToggleShortcut, () => {
                ...
                if (s === 'running')
                    stopTgws().catch(() => void 0);
                else
                    startTgws().catch(() => void 0);
            });
        }
        catch { /* noop */ }
    }
    if (cfg.zapretToggleShortcut) {
        try {
            globalShortcut.register(cfg.zapretToggleShortcut, () => {
                ...
                if (s === 'running')
                    stopZapret().catch(() => void 0);
                else
                    startZapret().catch(() => void 0);
            });
        }
        catch { /* noop */ }
    }
